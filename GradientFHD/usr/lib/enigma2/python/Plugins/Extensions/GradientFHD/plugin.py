...
def _gdbg(msg):
    try:
        import time
        with open("/home/root/logs/gradient_plugin_debug.log", "a") as f:
...
def main(session, **kwargs):
    try:
        _gdbg("Gradient plugin main entered")
    except Exception:
        pass
...
class GradientFHD_Config(Screen, ConfigListScreen):
    ...
    <screen name="GradientFHD_Config" position="center,center" size="1920,1080" title="GradientFHD Setup" flags="wfNoBorder" backgroundColor="transparent">
		...
		<eLabel text="GradientFHD Config" position="45,15" size="1110,60" font="Italic; 42" halign="left" valign="center" transparent="1" foregroundColor="gradient_foreground_selection" backgroundColor="background" borderColor="black" borderWidth="1" zPosition="1" />
		<widget name="config" position="30,90" size="1050,660" itemHeight="44" font="Gradient_Font;30" backgroundColor="gradient_background" foregroundColor="gradient_foreground" foregroundColorSelected="gradient_foreground_selection" scrollbarMode="showOnDemand" enableWrapAround="1" transparent="1" selectionPixmap="gfx/current/sel__1050.png" />
		<widget name="Picture" position="1100,100" size="782,484" alphatest="on" zPosition="2" />
		<ePixmap name="Line_Menu" position="20,760" size="1080,2" zPosition="10" pixmap="gfx/current/bg__1080x2_Line_Menu.png" alphatest="blend" />
    ...
    def getInitConfig(self):
        ...
        self.default_channel_selection_file = "channel_selection_Original.xml"
        ...
        self.channel_selection_file = "skin_user_channel_selection.xml"
        ...
        # channel_selection
        current, choices = self.getSettings(self.default_channel_selection_file, self.channel_selection_file)
        self.myAtileHD_channel_selection = NoSave(ConfigSelection(default=current, choices=choices))
        # myatile
    ...
    def createConfigList(self):
        ...
        self.set_channel_selection = getConfigListEntry(_("ChannelSelection Style:"), self.myAtileHD_channel_selection)
        ...
        if self.myAtileHD_active.value:
            ...
            if len(self.myAtileHD_channel_selection.choices) > 1:
                self.list.append(self.set_channel_selection)
    ...
    def changedEntry(self):
        ...
        elif self["config"].getCurrent() == self.set_channel_selection:
            self.setPicture(self.myAtileHD_channel_selection.value)
    ...
    def _getSelectedBackgroundColorName(self):
        """Return color name from selected background_color_skin xml."""
        try:
            val = self.myAtileHD_background_color_skin.value or ""
            name = path.basename(val)
            m = re.search(r'background_color_skin_Gradient_(.+)\.xml$', name, re.I)
            if m:
                return m.group(1)
        except Exception:
            pass
    ...
    def _copyTreeContents(self, src_dir, dst_dir):
        """Replace dst_dir contents with all files/folders from src_dir."""
        if not path.isdir(src_dir):
            raise Exception("Source folder not found: %s" % src_dir)

        if not path.isdir(dst_dir):
            makedirs(dst_dir)

        for fn in listdir(dst_dir):
            fp = path.join(dst_dir, fn)
            try:
                if path.isdir(fp) and not path.islink(fp):
                    shutil.rmtree(fp)
                else:
                    remove(fp)
            except Exception as e:
                _gdbg("BackgroundColor cleanup failed: %s (%s)" % (fp, e))

        copied = 0
        for fn in listdir(src_dir):
            s = path.join(src_dir, fn)
            d = path.join(dst_dir, fn)
            try:
                if path.isdir(s):
                ...
                _gdbg("BackgroundColor copy failed: %s -> %s (%s)" % (s, d, e))
        return copied

    def _applyBackgroundColorPixmaps(self):
        """Copy matching gfx/color_<name>/* into gfx/current/."""
        color_name = self._getSelectedBackgroundColorName()
        gfx_base = "/usr/share/enigma2/GradientFHD/gfx"
        src_dir = path.join(gfx_base, "color_%s" % color_name)
        dst_dir = path.join(gfx_base, "current")

        copied = self._copyTreeContents(src_dir, dst_dir)
        _gdbg("BackgroundColor pixmaps applied: %s (%d files)" % (color_name, copied))
        return color_name, copied
    ...
    def keyGreen(self):
        if self["config"].isChanged():
            ...
            try:
                color_name, copied = self._applyBackgroundColorPixmaps()
                AddPopup(tr(
                    "Background pixmaps switched to: %s" % color_name,
                    "Hintergrund-Pixmaps umgestellt auf: %s" % color_name
                ), type=MessageBox.TYPE_INFO, timeout=4)
                _gdbg("BackgroundColor switch OK: %s (%d files)" % (color_name, copied))
            except Exception as e:
                AddPopup(tr(
                    "Background pixmaps could not be switched: %s" % str(e),
                    "Hintergrund-Pixmaps konnten nicht umgestellt werden: %s" % str(e)
                ), type=MessageBox.TYPE_ERROR, timeout=6)
                _gdbg("BackgroundColor switch FAILED: %s" % str(e))
            # poster_movie_list
            self.makeSettings(self.myAtileHD_poster_movie_list, self.poster_movie_list_file)
            # channel_selection
            self.makeSettings(self.myAtileHD_channel_selection, self.channel_selection_file)
...
class GradientFHD_PrimeTimeSetup(Screen, ConfigListScreen):
    ...
    skin = """
	<screen name="GradientFHD_PrimeTimeSetup" position="center,center" size="1160,460" title="PrimeTime Settings" flags="wfNoBorder" backgroundColor="transparent">
		<widget source="Title" render="Label" position="20,0" size="1060,60" font="Italic; 42" halign="left" valign="center" transparent="1" foregroundColor="gradient_foreground_selection" backgroundColor="background" borderColor="black" borderWidth="1" zPosition="1" />
		<widget name="config" position="20,70" size="1120,315" font="Regular;30" scrollbarMode="showOnDemand" itemHeight="45" transparent="1" />
		<ePixmap name="menu_bg" position="center,center" size="1160,460" zPosition="-8" pixmap="gfx/current/bg___PrimeTime_Settings_BG.png" alphatest="blend" />
